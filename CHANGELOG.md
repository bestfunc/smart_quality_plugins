...
- 新增 `about-sync-data` —— **多客户端数据同步系统的产品百科**(user-invocable)。来自 PR #3 (refactor: 由原"独立 sync-data plugin"改注入 smart-tpm-{local,prod,test})。注:此次合入只在 3 变体目录,**_shared/skills/ 暂未同步**(后续补)。
